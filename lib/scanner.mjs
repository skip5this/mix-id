 ...
 * @param {string} opts.ffmpegCommand - Command or path to use for ffmpeg
 * @param {string} opts.ffprobeCommand - Command or path to use for ffprobe
...
export async function scan(file, opts = {}) {
  ...
  throwIfAborted(signal);
  const duration = opts.duration ?? await getDuration(file, {
    signal,
  ...
  try {
    for (let pos = start; pos + segment <= duration; pos += step) {
      ...
      try {
        if (!await extractAudioSegment(file, pos, segment, segPath, {
          signal,
