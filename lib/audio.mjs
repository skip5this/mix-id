...
export function resolveCommandPath(cmd, options = {}) {
  if (options.refresh) commandCache.delete(cmd);
...
export function hasCommand(cmd, options = {}) {
  const resolved = resolveCommandPath(cmd, options);
  return resolved !== cmd || canExecute(resolved);
}

function runProcess(cmd, args, opts = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(resolveCommandPath(cmd), args, {
...
export async function getDuration(file, opts = {}) {
  const { stdout } = await execFileAsync(resolveCommandPath(opts.ffprobeCommand ?? 'ffprobe'), [
    '-v', 'quiet',
    '-print_format', 'json',
    '-show_format',
    file,
...
export async function downloadURL(url, outputDir, opts = {}) {
  const ytDlpCommand = opts.ytDlpCommand ?? 'yt-dlp';
  if (!hasCommand(ytDlpCommand)) {
    throw new Error('yt-dlp is required for URL downloads. Install: brew install yt-dlp');
  }

  const ytDlp = resolveCommandPath(ytDlpCommand);
